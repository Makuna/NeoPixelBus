...
void IRAM_ATTR neoEspBitBangWritePixels(const uint8_t* pixels,
        const uint8_t* end, 
        uint8_t pin, 
        uint32_t t0h, 
        uint32_t t1h, 
        ...
        bool invert)
{
    ...
    uint32_t cyclesNext = 0;
        ...
        setValue = _BV(pin + 6); // + 6 due to using bit fields in out_w1ts
        clearValue = _BV(pin + 6); // + 6 due to using bit fields in out_w1tc
    #else
