...
//hack for testing 6ch bus settings, need to implement CW
...
RgbwwColor RgbwwColor::Dim(uint8_t ratio) const
{
    ...
    return RgbwwColor(_elementDim(R, ratio), _elementDim(G, ratio), _elementDim(B, ratio), _elementDim(WW, ratio), _elementDim(CW, ratio));
}

RgbwwColor RgbwwColor::Brighten(uint8_t ratio) const
{
    // specifically avoids float math
    return RgbwwColor(_elementBrighten(R, ratio), _elementBrighten(G, ratio), _elementBrighten(B, ratio), _elementBrighten(WW, ratio),  _elementBrighten(CW, ratio));
}
